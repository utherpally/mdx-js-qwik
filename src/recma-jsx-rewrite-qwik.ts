...
export const recmaJsxRewriteQwik: Plugin<any, Program> = () => {
  let id = 0;
  return (tree) => {
    const replacement = [];
    for (const _node of tree.body) {
      ...
      if (node.type === "FunctionDeclaration") {
        if (
          ...
          // Transform function MDXContent (props = {}) {}
          // to const MDXContent = _componentQrl(_inlinedQrl(function (props = {}) {}, 'symbolName'))
          // would allow using Qwik hook inside
          const symbolName = `${node.id?.name || "mdx"}_${id++}`; // Should unique symbolName is require in Qwik?
